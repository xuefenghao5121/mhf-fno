...
对于多核CPU系统（64核/128核/256核），我们提供了专门的性能优化来提升单核利用率。

### 问题

在大规模多核系统上直接运行训练脚本，常会遇到单核CPU利用率低（<30%）的问题，原因包括：
- 数据加载阻塞计算
- 不合理的线程配置导致超线程竞争
- 缺乏CPU绑定导致缓存命中率低
- 不必要的内存分配开销
...
我们在预训练脚本中添加了多项优化：
1. **多进程数据加载**：避免数据IO阻塞计算
2. **可配置OpenMP/MKL线程数**：避免超线程竞争
3. **CPU线程绑定**：提升缓存利用率
4. **内存分配优化**：减少不必要的零初始化
...
详细优化说明和调优指南请参考: [pretrained/PERFORMANCE_OPTIMIZATION.md](pretrained/PERFORMANCE_OPTIMIZATION.md)
