...
def main():
    ...
    print("  ✅ UNO 初始化成功")
    print(f"  fno_blocks: {len(model_uno.fno_blocks)} 个 block")
    
    # 打印每层结构
    for i, block in enumerate(model_uno.fno_blocks):
        conv = block.convs[0]
        if hasattr(conv, 'in_channels'):
            print(f"    Block {i}: in={conv.in_channels}, out={conv.out_channels}, modes={conv.n_modes}")
    ...
    replaced = []
    for i, block in enumerate(model_uno_mhf.fno_blocks):
        conv = block.convs[0]
        if hasattr(conv, 'in_channels') and conv.in_channels % 4 == 0 and conv.out_channels % 4 == 0:
            try:
                new_conv = MHFSpectralConv(
                    conv.in_channels, 
                    conv.out_channels, 
                    tuple(conv.n_modes),
                    n_heads=4
                )
                block.convs[0] = new_conv
                replaced.append(i)
                print(f"  ✅ 替换 Block {i}: in={conv.in_channels}, out={conv.out_channels}")
            except Exception as e:
                print(f"  ❌ Block {i} 替换失败: {e}")
        else:
            print(f"  ⏭️ Block {i}: 通道数不能被 4 整除 (in={conv.in_channels}, out={conv.out_channels})")
    ...
    if replaced:
        ...
        results.append({"name": "UNO+MHF", "params": params_mhf, "loss": loss_mhf, "time": time_mhf, "replaced": replaced})
    else:
        print("  ⚠️ 没有可替换的层")
        results.append({"name": "UNO+MHF", "error": "没有可替换的层"})
