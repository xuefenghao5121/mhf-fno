...
def load_data():
    """加载现有数据"""
    train_path = Path(__file__).parent.parent / 'data' / 'ns_train_32.pt'
    test_path = Path(__file__).parent.parent / 'data' / 'ns_test_32.pt'
...
def train_model(model, train_x, train_y, test_x, test_y, epochs=50, name="Model"):
    """训练模型"""
...
def main():
    print("="*70)
    print("NS 优化测试 - 保守配置 vs 原始配置 (500 样本)")
    print("="*70)
    ...
    mhf_cons = create_mhf_fno_with_attention(
        ...
        attention_layers=[0], attn_dropout=0.0
    )
    ...
    mhf_orig = create_mhf_fno_with_attention(
        ...
        attention_layers=[0, 2], attn_dropout=0.0
    )
    ...
    output_file = Path(__file__).parent.parent / 'ns_opt_results_500samples.json'
    ...
    print(f"\n测试配置: n_train={train_x.shape[0]}, epochs={epochs}")
    ...
    if improvement_orig >= 0:
        print(f"✅ 原始配置成功！MHF-FNO (原始) 优于 FNO {improvement_orig:.2f}%")
    else:
        print(f"⚠️  原始配置未能超越 FNO ({improvement_orig:.2f}%)")
    ...
    with open(log_file, 'a') as f:
        ...
        f.write(f"配置: epochs={epochs}, n_modes={n_modes}, hidden={hidden_channels}\n")
        f.write(f"FNO: {fno_loss:.4f} (epoch {fno_epoch})\n")
        f.write(f"MHF (保守 mhf_layers=[0]): {mhf_cons_loss:.4f} (epoch {mhf_cons_epoch}), {improvement_cons:+.2f}%\n")
        f.write(f"MHF (原始 mhf_layers=[0,2]): {mhf_orig_loss:.4f} (epoch {mhf_orig_epoch}), {improvement_orig:+.2f}%\n")
        f.write(f"结论: 保守配置 {'优于' if improvement_cons > improvement_orig else '未优于'} 原始配置\n")
