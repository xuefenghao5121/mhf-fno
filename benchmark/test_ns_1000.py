#!/usr/bin/env python3
"""
NS 数据集测试 - n=1000 数据量扩展

目标: 验证数据量扩展对 NS 效果的改善
"""
...
def train_model(model, train_x, train_y, test_x, test_y, epochs=50):
    """训练模型"""
...
def main():
    ...
    t0 = time.time()
    loss_fno = train_model(model_fno, train_x, train_y, test_x, test_y, epochs=50)
    ...
    t0 = time.time()
    loss_mhf = train_model(model_mhf, train_x, train_y, test_x, test_y, epochs=50)
    ...
    # 保存结果
    output_path = Path(__file__).parent.parent / 'ns_test_1000_results.json'
