...
def main():
    ...
    # =========================================
    print("\n" + "=" * 70)
    print(f"基准测试: FNO ({epochs} epochs)")
    print("=" * 70)
    ...
    t0 = time.time()
    fno_loss, fno_epoch, _ = train_and_eval(model_fno, train_x, train_y, test_x, test_y, epochs, batch_size, lr, device)
    ...
    t0 = time.time()
    iter1_loss, iter1_epoch, _ = train_and_eval(model_iter1, train_x, train_y, test_x, test_y, epochs, batch_size, lr, device)
    ...
    t0 = time.time()
    heads2_loss, heads2_epoch, _ = train_and_eval(model_2heads, train_x, train_y, test_x, test_y, epochs, batch_size, lr, device)
    ...
    t0 = time.time()
    full_coda_loss, full_coda_epoch, _ = train_and_eval(model_full_coda, train_x, train_y, test_x, test_y, epochs, batch_size, lr, device)
    ...
    # =========================================
    # P0 测试 3: 组合 (n_heads=2 + 全层 CoDA)
    # =========================================
    print("\n" + "=" * 70)
    print("P0 测试 3: 组合 (n_heads=2 + 全层 CoDA)")
    print("=" * 70)
    print("预期: 多改进组合效果")
    ...
    t0 = time.time()
    combo_loss, combo_epoch, _ = train_and_eval(model_combo, train_x, train_y, test_x, test_y, epochs, batch_size, lr, device)
    ...
    results['tests']['CoDA-n_heads2-full'] = {
        'params': params_combo,
