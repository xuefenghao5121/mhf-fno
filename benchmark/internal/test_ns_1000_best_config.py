...
def main():
    print("="*60)
    print("NS 数据集测试 - README 最佳配置")
    print("="*60)
    print("配置: n_modes=(12,12), hidden=32, layers=3, heads=4")
    print("      mhf_layers=[0,2], bottleneck=4, gate_init=0.1")
    print("="*60)
    ...
    # ========== MHF+CoDA (最佳配置) ==========
    print(f"\n{'='*60}")
    print("测试 MHF+CoDA (最佳配置)")
    print(f"{'='*60}")
    
    torch.manual_seed(42)
    from mhf_fno import MHFFNOWithAttention
    model_mhf = MHFFNOWithAttention.best_config(
        n_modes=n_modes,
        hidden_channels=hidden_channels,
        in_channels=in_channels,
        out_channels=out_channels,
    ...
    # 保存结果
    output_path = Path(__file__).parent.parent / 'ns_test_best_config_results.json'
