...
class SpatialEngram(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 量化并取模
        indices = (pooled.abs().mean(dim=1) * 100).long() % self.num_patterns
        indices = indices.clamp(0, self.num_patterns - 1)  # 确保索引在范围内
        
        # 查表
        embeddings = self.embedding_table(indices)  # (B, H, W, C)
        output = embeddings.permute(0, 3, 1, 2)  # (B, C, H, W)
...
class SpectralEngram(nn.Module):
    ...
    def __init__(
        ...
        self.freq_weight = nn.Parameter(
            torch.randn(channels, self.modes_x, self.modes_y, dtype=torch.cfloat) * 0.01
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_freq[:, :, :m_x, :m_y] = x_freq[:, :, :m_x, :m_y] * self.freq_weight[:, :m_x, :m_y].unsqueeze(0)
        
        # 加入调制
        out_freq = out_freq * mod.view(B, C, 1, 1)
