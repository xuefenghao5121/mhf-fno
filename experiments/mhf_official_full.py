...
def train_model(model, train_x, train_y, test_x, test_y, epochs=100, batch_size=32, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 20 == 0:
            print(f"Epoch {epoch+1}: Train={avg_train:.4f}, Test={test_loss:.4f}")
    
    final_loss = history['test'][-1]
...
def main():
    print("\n" + "=" * 70)
    print(" MHF-FNO 官方框架完整测试")
    print(" 训练样本: 1000 | Epochs: 100 | Batch: 32")
    print("=" * 70)
    ...
    loss_fno, time_fno, hist_fno = train_model(
        model_fno, train_x, train_y, test_x, test_y, epochs, batch_size
    )
    
    print(f"\n最终 Test Loss: {loss_fno:.4f}")
    ...
    loss_mhf, time_mhf, hist_mhf = train_model(
        model_mhf, train_x, train_y, test_x, test_y, epochs, batch_size
    )
    
    print(f"\n最终 Test Loss: {loss_mhf:.4f}")
    ...
    # 改进分析
    p_change = (params_mhf - params_fno) / params_fno * 100
    l_change = (loss_mhf - loss_fno) / loss_fno * 100
    t_change = (time_mhf - time_fno) / time_fno * 100
    
    print(f"\n📊 MHF-FNO vs FNO:")
    print(f"   参数量: {p_change:+.1f}%")
    print(f"   L2误差: {l_change:+.1f}%")
    print(f"   训练时间: {t_change:+.1f}%")
    ...
    # 保存结果
    save_results = {
        'FNO': {'params': params_fno, 'loss': loss_fno, 'time': time_fno},
        'MHF-FNO': {'params': params_mhf, 'loss': loss_mhf, 'time': time_mhf}
    }
    with open('benchmark_results_official.json', 'w') as f:
        json.dump(save_results, f, indent=2)
    print(f"\n结果已保存到 benchmark_results_official.json")
